...
export function ExportDialog({ filename, allColumns, fetchRows, label = "Export CSV" }: Props) {
  ...
  async function handleExport() {
    ...
    try {
      ...
      exportCSV(filename, visible.map((c) => c.label), rows);
      setOpen(false);
    } catch (e) {
        ...
        <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center" onClick={() => setOpen(false)}>
          <div className="bg-white rounded-xl shadow-2xl border border-[#e2e8f0] w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#e2e8f0]">
              <h2 className="text-lg font-bold text-[#0e212c]">Export — Select Columns</h2>
            ...
            <div className="p-6 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold text-[#64748b] uppercase tracking-wider">{selected.length} of {allColumns.length} selected</p>
            ...
            <div className="px-6 py-4 border-t border-[#e2e8f0] flex gap-3">
              ...
              <button onClick={handleExport} disabled={selected.length === 0 || loading}
                ...
                {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Exporting...</> : <><Download className="h-4 w-4" /> Export ({selected.length} cols)</>}
              </button>
            </div>
          </div>
        </div>
