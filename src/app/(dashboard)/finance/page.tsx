...
export default function FinancePage() {
  ...
  const topPaginated = topProducts.slice(
    (topPage - 1) * topPerPage,
    topPage * topPerPage,
  );
    ...
    <div className="space-y-6">
      <div>
        <PageHeader
          ...
          subtitle="Revenue, expenses, profit & cash flow analysis"
        />
      </div>
        ...
        <>
            ...
            {[
              {
                icon: TrendingUp,
                label: "Gross Sales",
                value: `${fin.grossSales.toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
                change: fin.comparison.grossChange,
              ...
              },
              {
                icon: DollarSign,
                label: "Gross Revenue",
                value: `${fin.grossRevenue.toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
                change: fin.comparison.netChange,
              ...
              },
              {
                icon: ShoppingCart,
                label: "Restocks Cost",
                value: `${fin.restocksTotal.toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
              ...
              },
              {
                icon: Undo2,
                label: "Returns Loss",
                value: `${fin.returnsTotal.toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
              ...
              },
              {
                icon: AlertTriangle,
                label: "Damages Loss",
                value: `${fin.damagesTotal.toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
              ...
              },
              {
                icon: TrendingUp,
                label: "Net Revenue",
                value: `${fin.netRevenue.toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
                profit: fin.netRevenue >= 0,
              ...
              <div
                key={i}
                className="col-span-12 sm:col-span-6 lg:col-span-2 bg-white/90 backdrop-blur-sm rounded-xl border border-[#e2e8f0] p-5 hover:shadow-lg hover:shadow-black/5 hover:bg-white transition-all duration-300 hover:-translate-y-0.5 group"
              >
                ...
                <p
                  className={`text-2xl font-bold tracking-tight ${card.profit !== undefined ? (card.profit ? "text-emerald-600" : "text-rose-600") : "text-[#0e212c]"}`}
                >
                  {card.value}
                </p>
                <p className="text-xs text-[#94a3b8] mt-1.5 flex items-center gap-1">
                  {card.change !== null && card.change !== undefined ? (
                    <span
                      className={`flex items-center gap-0.5 font-semibold ${card.change >= 0 ? "text-emerald-500" : "text-rose-500"}`}
                    >
                      {card.change >= 0 ? (
          ...
          <div className="grid grid-cols-12 gap-4">
            {[
              ...
              },
              {
                icon: RotateCcw,
                label: "Returns",
                value: fin.returnCount,
                color: "bg-amber-50 text-amber-600",
              },
              {
                icon: ShoppingCart,
                label: "Restocks",
                value: fin.restockCount,
                color: "bg-blue-50 text-blue-600",
              },
              {
                icon: AlertTriangle,
                label: "Damages",
                value: fin.damageCount,
                color: "bg-rose-50 text-rose-600",
              },
              {
                icon: Package,
                label: "Adjustments",
                value: fin.adjustmentCount,
                color: "bg-violet-50 text-violet-600",
              },
              {
                icon: Ban,
                label: "Cancelled",
                value: fin.cancelledCount,
                color: "bg-rose-50 text-rose-600",
              },
            ].map((card, i) => (
              <div
                key={i}
                className="col-span-6 sm:col-span-3 lg:col-span-3 bg-white rounded-xl border border-[#e2e8f0] p-4 flex items-center gap-4 hover:shadow-md transition-all"
              >
                <div
                  className={`w-10 h-10 rounded-lg ${card.color} flex items-center justify-center shrink-0`}
                >
                  <card.icon className="h-4 w-4" />
                </div>
                <div>
                  <p className="text-xs text-[#64748b]">{card.label}</p>
                  <p className="text-lg font-bold text-[#0e212c]">
                    {card.value}
                  </p>
                </div>
              </div>
          ...
          <div className="grid grid-cols-12 gap-5">
            <div className="col-span-12 sm:col-span-6 bg-white rounded-xl border border-[#e2e8f0] p-6">
              <h3 className="text-sm font-bold text-[#0e212c] mb-4 flex items-center gap-2">
                <BarChart3 className="h-4 w-4 text-[#fd761a]" /> Top Products by
                Revenue ({fin.period.label})
              </h3>
                ...
                <>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm table-fixed">
                      <thead>
                        <tr className="text-[10px] font-semibold text-[#94a3b8] uppercase tracking-wider border-b border-[#e2e8f0]">
                          <th className="text-left pb-3 font-semibold w-2/5">
                            Product
                          </th>
                          <th className="text-right pb-3 font-semibold w-1/5">
                            Qty Sold
                          </th>
                          <th className="text-right pb-3 font-semibold w-1/5">
                            Revenue
                      ...
                      <tbody className="divide-y divide-[#e2e8f0]">
                        {topPaginated.map((p: any, i: number) => {
                          ...
                          return (
                            ...
                            >
                              <td
                                className="py-3 font-medium text-[#0e212c] truncate"
                                title={p.name}
                              >
                                {p.name}
                              </td>
                              ...
                              <td className="py-3 text-right font-mono font-semibold text-[#0e212c]">
                              ...
                              <td className="py-3 pl-4">
                                <div className="h-2.5 bg-[#f1f5f9] rounded-full overflow-hidden">
                                  <div
                                    className="h-full bg-gradient-to-r from-[#fd761a] to-[#e56600] rounded-full"
                                    style={{ width: `${barW}%` }}
                                  />
                                </div>
                              </td>
                  ...
                  {topTotalPages > 1 && (
                    <div className="flex items-center justify-between pt-4 border-t border-[#e2e8f0] mt-4">
                      <span className="text-xs text-[#94a3b8]">
                        {topProducts.length} total products
                      </span>
            ...
            <div className="col-span-12 sm:col-span-6 bg-white rounded-xl border border-[#e2e8f0] p-6">
              <h3 className="text-sm font-bold text-[#0e212c] mb-4 flex items-center gap-2">
                <PieChart className="h-4 w-4 text-[#fd761a]" /> Payment Methods
              </h3>
              {fin.paymentBreakdown.length > 0 ? (
                <div className="space-y-3">
                  {fin.paymentBreakdown.map((pm: any, i: number) => {
                    const pct =
                      fin.grossRevenue > 0
                        ? ((pm.total / fin.grossRevenue) * 100).toFixed(1)
                        : 0;
                    return (
                      <div key={i}>
                        ...
                        <div className="w-full h-2 bg-[#f1f5f9] rounded-full overflow-hidden">
                          <div
                            className="h-full bg-gradient-to-r from-[#fd761a] to-[#e56600] rounded-full transition-all duration-500"
                            style={{ width: `${pct}%` }}
                          />
                        </div>
          ...
          <div className="bg-white rounded-xl border border-[#e2e8f0] p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-[#0e212c] flex items-center gap-2">
                <LineChart className="h-4 w-4 text-[#fd761a]" /> Cash Flow
                Timeline ({fin.period.label})
              </h3>
            </div>
            <div className="mt-2">
            ...
            <div className="flex items-center justify-center gap-6 mt-4 pt-4 border-t border-[#f1f5f9] text-[10px] font-semibold text-[#64748b] uppercase tracking-wider">
              <span className="flex items-center gap-2 px-2 py-1 rounded-md hover:bg-emerald-50 transition-colors">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-400/50 shadow-sm" />{" "}
                Revenue
              </span>
              <span className="flex items-center gap-2 px-2 py-1 rounded-md hover:bg-rose-50 transition-colors">
                <span className="w-2.5 h-2.5 rounded-full bg-rose-400/50 shadow-sm" />{" "}
                Expenses
              </span>
              <span className="flex items-center gap-2 px-2 py-1 rounded-md hover:bg-emerald-50 transition-colors">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-500/70 shadow-sm" />{" "}
                Net Profit
              </span>
            </div>
          </div>

          <p className="text-[10px] text-[#94a3b8] text-center font-medium opacity-70 tracking-widest uppercase">
            {fin.period.label}
          </p>
        </>
      ) : (
