...
export default function OrdersPage() {
  ...
  function openEdit(t: TxnWithItems) {
    ...
    setEditItems(t.items.map((i) => ({ productId: i.productId!, quantity: i.quantity!, unitPrice: Number(i.unitPrice!), totalPrice: Number(i.totalPrice!) })));
  }
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        {filtered.map((order) => {
          ...
          return (
            <div key={order.id} className="bg-white border border-[#e2e8f0] rounded-xl overflow-hidden hover:shadow-md transition-shadow">
              <div className="p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0 grid grid-cols-5 gap-4 text-sm">
                  ...
                  <div>
                    <p className="text-[10px] font-semibold text-[#94a3b8] uppercase tracking-wider">Items</p>
                    <p className="text-[#0e212c] font-medium mt-0.5">{order.items.length} item{order.items.length !== 1 ? "s" : ""}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-[10px] font-semibold text-[#94a3b8] uppercase tracking-wider">Date</p>
                    <p className="text-[#64748b] mt-0.5">{new Date(order.transactionDate).toLocaleDateString("en-PH", { month: "short", day: "numeric" })}</p>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                  <button onClick={() => quickStatusChange(order.id, "Completed")}
                    className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all" title="Mark Completed">
                    <CheckCircle className="h-4 w-4" />
                  </button>
                  <button onClick={() => quickStatusChange(order.id, "Cancelled")}
                    className="p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition-all" title="Cancel">
                    <XCircle className="h-4 w-4" />
                  </button>
                  <button onClick={() => openEdit(order)}
                    className="p-2 text-[#64748b] hover:bg-[#f1f5f9] rounded-lg transition-all" title="Edit Order">
              ...
              {isExpanded && !isEditing && (
                <div className="border-t border-[#e2e8f0] bg-[#f8fafc] p-4">
                  ...
                  <div className="flex gap-2 mt-3">
                    ...
                    <span className="text-[10px] font-semibold text-[#94a3b8] uppercase ml-4">Contact:</span>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16 text-[#94a3b8]">
            ...
            <p className="text-xs mt-1">No ongoing purchase orders at this time</p>
          </div>
        )}
      </div>
    </div>
