              ...
              {!result && (
                <div className="space-y-4">
                  <div className="border-2 border-dashed border-[#e2e8f0] rounded-xl p-6 text-center hover:border-[#fd761a] transition-colors"
                    onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) { setFile(f); f.text().then(parseCSV); } }}>
                    <input ref={inputRef} type="file" accept=".csv" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) { setFile(f); f.text().then(parseCSV); } }} />
                    {file ? (
