...
export function UsersClient({ users: initialUsers, currentUserRole }: Props) {
  ...
  async function handleToggle() {
    ...
    try {
      ...
      router.refresh();
      toast.success(
        toggleTarget.isActive ? "User deactivated" : "User activated",
      );
    } catch (err: any) {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        subtitle="Manage system users — add, edit, activate or deactivate accounts. Assign roles and control access."
      />
            ...
            fetchRows={async (selectedColumns) =>
              filtered.map((u) =>
                selectedColumns.map((key) => {
                  ...
                  if (key === "lastLogin")
                    return u.lastLogin
                      ? new Date(u.lastLogin).toLocaleString("en-PH", {
            ...
            <tbody className="divide-y divide-[#e2e8f0]">
              {paginated.map((u, i) => (
                ...
                >
                  ...
                  <td className="p-4 text-[#64748b] font-mono text-sm">
                    {u.username}
                  </td>
                  ...
                  <td className="p-4 text-center">
                    <span
                      className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${u.isActive ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-rose-50 text-rose-700 border border-rose-200"}`}
                    >
            ...
            <form onSubmit={handleAdd} className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setShowAdd(false)}
                  className="px-5 py-2.5 border border-[#e2e8f0] rounded-lg text-sm font-medium text-[#64748b] hover:bg-[#f8fafc] transition-colors"
                >
                  Cancel
                </button>
                <button
