...
export default function RegisterPage() {
  ...
  useEffect(() => {
    setVerifyToken(new URLSearchParams(window.location.search).get("verifyToken") || "");
  }, []);
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-xl p-8 shadow-2xl shadow-black/10 border border-white/20">
          ...
          {!verifyToken && !verified && (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input type="email" value={form.email} onChange={(e) => update("email", e.target.value)} className="w-full rounded-lg border border-outline px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" required />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-sm font-medium">Password</label>
                  <div className="relative">
                    <input type={showPw ? "text" : "password"} value={form.password} onChange={(e) => update("password", e.target.value)} className="w-full rounded-lg border border-outline px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" required />
                    <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#94a3b8] hover:text-[#64748b] transition-colors">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    <input type={showConfirmPw ? "text" : "password"} value={form.confirmPassword} onChange={(e) => update("confirmPassword", e.target.value)} className="w-full rounded-lg border border-outline px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" required />
                    <button type="button" onClick={() => setShowConfirmPw(!showConfirmPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#94a3b8] hover:text-[#64748b] transition-colors">
          ...
          {verifyToken && !verified && (
            <form onSubmit={handleVerify} className="space-y-4">
              ...
              <button type="submit" disabled={loading || !code} className="w-full py-2.5 bg-secondary text-on-secondary rounded-lg font-semibold disabled:opacity-50">
          ...
          {verified && (
            <div className="space-y-4 text-center">
              <p className="text-sm text-[#0e212c]">Your account is verified. You can now sign in.</p>
              <Link href="/login" className="inline-flex w-full justify-center py-2.5 bg-secondary text-on-secondary rounded-lg font-semibold">Go to Login</Link>
            </div>
          )}
