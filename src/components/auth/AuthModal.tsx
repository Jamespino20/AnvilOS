...
export function AuthModal({ 
  ...
  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 z-[200] bg-black/50 backdrop-blur-sm flex items-center justify-center" onClick={onClose}>
          <div className="bg-white w-full max-w-[440px] mx-4 rounded-xl shadow-2xl border border-[#e2e8f0] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex flex-col relative">
              ...
              <div className="bg-gradient-to-br from-[#0e212c] to-[#1a3a4a] px-8 py-12 text-white relative border-b border-white/5 overflow-hidden">
                ...
                <button onClick={onClose} className="absolute right-4 top-6 rounded-sm opacity-50 hover:opacity-100 transition-opacity">
                  <X className="h-5 w-5" />
                </button>
                
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-1 z-10 relative">
                    ...
                    {activeTab === "login" ? (
                      <form onSubmit={handleLogin} className="space-y-4">
                        <div className="space-y-2">
                          <Label htmlFor="login-email" className="text-xs font-semibold text-[#64748b] uppercase tracking-wider">Merchant Identity / Username</Label>
                          <Input id="login-email" type="text" required placeholder="operator01"
                            value={formData.email} onChange={(e) => updateField("email", e.target.value)}
                            className="text-[#0e212c] border-[#e2e8f0] focus:border-[#fd761a]" disabled={isLoading} />
                        </div>
                        <div className="space-y-2">
                          <div className="flex justify-between items-center">
                            <Label htmlFor="login-pass" className="text-xs font-semibold text-[#64748b] uppercase tracking-wider">Security Protocol / Password</Label>
                          </div>
                        ...
                        <Button type="submit" disabled={isLoading}
                          ...
                          {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Proceed to Dashboard →</>}
                        </Button>
                      </form>
                    ) : (
                      <form onSubmit={handleRegister} className="space-y-4">
                        <div className="space-y-2">
                          <Label htmlFor="reg-name" className="text-xs font-semibold text-[#64748b] uppercase tracking-wider">Legal Entity / Merchant ID</Label>
                          <Input id="reg-name" type="text" required placeholder="operator01"
                            value={formData.sellerName} onChange={(e) => updateField("sellerName", e.target.value)}
                            className="text-[#0e212c] border-[#e2e8f0] focus:border-[#fd761a]" disabled={isLoading} />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="reg-email" className="text-xs font-semibold text-[#64748b] uppercase tracking-wider">Communication Link / Email</Label>
                          <Input id="reg-email" type="email" required placeholder="enterprise@company.com"
                            value={formData.email} onChange={(e) => updateField("email", e.target.value)}
                            className="text-[#0e212c] border-[#e2e8f0] focus:border-[#fd761a]" disabled={isLoading} />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="reg-pass" className="text-xs font-semibold text-[#64748b] uppercase tracking-wider">Access Sequence / Password</Label>
                          <Input id="reg-pass" type="password" required placeholder="Minimum 8 characters"
                            value={formData.password} onChange={(e) => updateField("password", e.target.value)}
                            className="text-[#0e212c] border-[#e2e8f0] focus:border-[#fd761a]" disabled={isLoading} />
                        </div>
                        <Button type="submit" disabled={isLoading}
                          ...
                          {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Forge New Profile →</>}
                        </Button>
                      </form>
                    )}
                  </motion.div>
              ...
              <div className="bg-[#f8fafc] px-8 py-4 border-t border-[#e2e8f0] flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className={`w-1.5 h-1.5 rounded-full ${isLoading ? "bg-amber-500 animate-pulse" : "bg-green-500 animate-pulse"}`} />
                  <span className="text-[9px] font-bold text-[#94a3b8] uppercase tracking-widest">
                    {isLoading ? "Processing Link..." : "System Online"}
                  </span>
                </div>
                <span className="text-[9px] font-mono text-[#94a3b8] opacity-50">AV-GATE-v2.5.0</span>
              </div>
            </div>
          </div>
        </div>
