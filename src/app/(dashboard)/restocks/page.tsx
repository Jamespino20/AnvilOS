...
export default function RestocksPage() {
  ...
  const filteredProducts = useMemo(
    () =>
      products.filter((p) =>
        p.productName.toLowerCase().includes(restockSearch.toLowerCase()),
      ),
    ...
    setCart((prev) => {
      ...
      return [
        ...prev,
        {
          ...
          quantity: 1,
          costPrice: product.unitPrice ? Number(product.unitPrice) : "",
        },
      ];
    });
  }
  ...
  function commitQtyEdit(productId: number) {
    ...
    if (!isNaN(val) && val > 0) {
      setCart((prev) =>
        prev.map((i) =>
          i.productId === productId ? { ...i, quantity: val } : i,
        ),
      );
    }
    setEditingQty(null);
  ...
  if (loading)
    return (
      <div className="space-y-5">
        <PageHeader title="Restocks" subtitle="Loading..." />
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Restocks"
        subtitle={`${total} restock record${total !== 1 ? "s" : ""} — track and process inventory replenishment.`}
      />
      ...
      <div className="bg-white border border-[#e2e8f0] rounded-xl p-4 flex flex-col lg:flex-row gap-4 items-center">
        <div className="relative w-full lg:flex-1 min-w-0 sm:min-w-[200px]">
          ...
          <input
            ...
            placeholder="Search by supplier name, receipt #..."
                ...
                selectedColumns.map((key) => {
                  ...
                  if (key === "grandTotal") {
                    const total = r.items.reduce(
                      (s, i) =>
                        s +
                        Number(i.costPrice || i.unitPrice || 0) *
                          (i.quantity || 0),
                      0,
                    );
                  ...
                  if (key === "transactionDate")
                    return new Date(r.transactionDate).toLocaleString("en-PH");
      ...
      <div className="space-y-2">
        {restocks.map((r) => {
          ...
          return (
            <div
              key={r.id}
              className={`bg-white border rounded-xl overflow-hidden hover:shadow-md transition-shadow ${isProcessed ? "border-emerald-200" : "border-[#e2e8f0]"}`}
            >
              <div className="p-4 flex items-center gap-4">
                <div
                  className={`w-11 h-11 rounded-lg flex items-center justify-center shrink-0 ${isProcessed ? "bg-emerald-50 text-emerald-600" : "bg-purple-50 text-purple-600"}`}
                >
                  {isProcessed ? (
                ...
                <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-[#64748b] mt-0.5">
                      {new Date(r.transactionDate).toLocaleString("en-PH", {
                        month: "short",
                        day: "numeric",
                        year: "numeric",
                        hour: "2-digit",
                  ...
                  <div>
                    ...
                    <p
                      className={`mt-0.5 text-xs font-semibold ${isProcessed ? "text-emerald-600" : "text-amber-600"}`}
                    >
        ...
        {restocks.length === 0 && (
          <div className="text-center py-16 text-[#94a3b8]">
            ...
            <p className="text-xs mt-1">
              Create a new restock to start tracking inventory replenishment.
            </p>
          </div>
        )}
      </div>
      ...
      {showNew && (
        <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center">
          <div
            className="bg-white rounded-xl shadow-2xl border border-[#e2e8f0] w-full max-w-6xl mx-4 h-[85vh] flex flex-col"
          ...
          >
            ...
            <div className="flex flex-col lg:flex-row gap-0 h-full overflow-hidden">
              ...
              <div className="lg:flex-[1] p-8 overflow-y-auto border-b lg:border-b-0 lg:border-r border-[#e2e8f0] space-y-4">
                ...
                {productViewMode === "list" ? (
                    ...
                    {filteredProducts.map((p) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {inCart && (
                            <span className="text-xs text-[#fd761a] font-semibold">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        ...
                        >
                          {imgUrl ? (
                            <img
                              src={imgUrl}
                              alt=""
                              className="w-full h-16 object-cover rounded-lg mb-2 border border-[#e2e8f0]"
                            />
                          ) : (
                          ...
                          <p className="text-xs font-medium text-[#0e212c] leading-snug line-clamp-2">
                            {p.productName}
                          </p>
                          ...
                          {inCart && (
                            <p className="text-[10px] text-[#fd761a] font-semibold mt-1">
                              {inCart.quantity} in cart
                            </p>
                          )}
                        </button>
                    ...
                    {filteredProducts.length === 0 && (
                      <div className="col-span-3 text-center py-8 text-[#94a3b8] text-sm">
                        No products match your search
                      </div>
                    )}
                  </div>
              ...
              {/* Cart area */}
              <div className="lg:flex-[1] p-8 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 mb-5">
                  {cart.map((item) => (
                    <div
                      key={item.productId}
                      className="bg-[#f8fafc] rounded-lg p-3 space-y-2 hover:bg-[#f1f5f9] transition-colors group"
                    >
                      <div className="flex items-center gap-2">
                        <p className="text-[13px] font-medium text-[#0e212c] leading-tight flex-1">
                          {item.productName}
                        </p>
                        <button
                          type="button"
                          onClick={() => removeFromCart(item.productId)}
                          className="w-7 h-7 flex items-center justify-center text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-md transition-all shrink-0 opacity-0 group-hover:opacity-100"
                        >
                      ...
                      <div className="flex items-center gap-4">
                              ...
                              setCart((prev) =>
                                prev.map((i) =>
                                  i.productId === item.productId
                                    ? { ...i, costPrice: e.target.value }
                                    : i,
                                ),
                              );
                            }}
                            className="w-24 h-10 px-2.5 text-xs text-right font-mono border border-[#e2e8f0] rounded-lg focus:outline-none focus:border-[#fd761a]"
                            placeholder="0.00"
                          />
                        </div>
                        <div className="flex items-center gap-1">
                          ...
                          {editingQty === item.productId ? (
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                startQtyEdit(item.productId, item.quantity)
                              }
                              className="min-w-[40px] h-10 text-center text-xs font-semibold text-[#0e212c] px-1.5 hover:bg-white rounded-lg transition-colors"
                        ...
                        <div className="ml-auto text-right">
                          ...
                          <p className="text-xs font-bold text-[#0e212c]">
                            {(
                ...
                >
                  {submitting ? (
                    ...
                    <>
                      <Truck className="h-4 w-4" /> Record Restock ({totalItems}{" "}
                      items)
                    </>
                  )}
                </button>
      ...
      {expandedId !== null &&
        (() => {
          ...
          const grandTotal = restock.items.reduce(
            (s, i) =>
              s + Number(i.costPrice || i.unitPrice || 0) * (i.quantity || 0),
            0,
          );
          return (
            <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
              ...
              >
                <div className="flex items-center justify-between px-6 py-4 border-b border-[#e2e8f0]">
                  <div>
                    <h2 className="text-lg font-bold text-[#0e212c] flex items-center gap-2">
                      <ArrowDownUp className="h-5 w-5 text-[#fd761a]" /> Restock
                      #{restock.receiptNumber}
                    </h2>
                    <p className="text-xs text-[#64748b] mt-1">
                      {restock.buyerName} &middot;{" "}
                      {new Date(restock.transactionDate).toLocaleString(
                        "en-PH",
                        {
                          month: "short",
                          day: "numeric",
                ...
                <div className="overflow-y-auto p-6 space-y-4">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-[#e2e8f0]">
                      {restock.items.map((item) => {
                        ...
                        return (
                          <tr key={item.id}>
                            <td className="py-2.5 text-[#0e212c] font-medium">
                              {item.productName ||
                                prod?.productName ||
                                `#${item.productId}`}
                            </td>
                            ...
                            <td className="py-2.5 text-right text-[#64748b] font-mono">
                              {cost.toLocaleString("en-PH", {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              })}
                            </td>
                            <td className="py-2.5 text-right text-[#0e212c] font-semibold font-mono">
                              {(cost * (item.quantity || 0)).toLocaleString(
                                "en-PH",
                                {
                                  minimumFractionDigits: 2,
                                  maximumFractionDigits: 2,
                    ...
                    <tfoot>
                      <tr className="border-t border-[#e2e8f0]">
                        <td
                          colSpan={3}
                          className="pt-3 text-right text-sm font-semibold text-[#0e212c]"
                        >
                          Grand Total
                        </td>
                        <td className="pt-3 text-right text-sm font-bold text-[#fd761a] font-mono">
                  ...
                  {(restock.deliveryRef ||
                    restock.deliveryNotes ||
                    restock.delivererName) && (
