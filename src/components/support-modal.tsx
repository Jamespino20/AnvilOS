...
export function SupportModal({ open, onClose }: Props) {
  ...
  const links = [
    ...
    { icon: MessageCircle, label: "Live Chat", desc: "Chat with support team", href: "#" },
    { icon: Mail, label: "Email Support", desc: "support@anvilos.com", href: "mailto:support@anvilos.com" },
  ];
    ...
    <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl border border-[#e2e8f0] w-full max-w-sm mx-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-3">
          {links.map((link) => (
            <a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer"
              ...
              <div>
                <p className="text-sm font-semibold text-[#0e212c]">{link.label}</p>
        ...
        <div className="px-6 py-4 border-t border-[#e2e8f0] text-center">
          <p className="text-xs text-[#94a3b8]">Response time: within 24 hours</p>
        </div>
      </div>
    </div>
  );
