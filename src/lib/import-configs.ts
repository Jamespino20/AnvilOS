export const IMPORT_CONFIGS: Record<string, { label: string; columns: { key: string; label: string; type: "string" | "number" | "date" | "boolean"; required?: boolean; enum?: string[] }[] }> = {
  products: {
    ...
    columns: [
      ...
      { key: "isFastMoving", label: "Fast Moving", type: "boolean" },
      { key: "sellByWeight", label: "Sell by Weight", type: "boolean" },
      { key: "sellByBox", label: "Sell by Box", type: "boolean" },
      { key: "boxQuantity", label: "Qty per Box", type: "number" },
    ],
  },
  inventory: {
    label: "Inventory",
